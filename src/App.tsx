...
const App: React.FC = () => {
  ...
  const handleMaxDistanceChange = async (event: React.ChangeEvent<HTMLInputElement>) => {
    // setMaxDistance(Number(event.target.value));
    // console.log(maxDistance)
    try {
      setMaxDistance(Number(event.target.value))
      // Отправляем запрос на поиск границ
      await axios.post('/clusters/settings', { maxDistance: maxDistance });
    } catch (error) {
      console.error('Ошибка при изменении параметров кластеризации', error);
    }
  };
